...
def eval_student_on_random_envs(policy, env, count: int, eval_dir, iters, max_eval_steps=None, verbose=False):
    ...
    save(
        obj=[e.domain_param for e in random_envs],
        name="random_env_params.pkl",
        save_dir=eval_dir,
        suffix=datetime.now().strftime("%Y-%m-%d_%H:%M:%S"),
    )

    a_pool = multiprocessing.Pool(processes=4)
    su = a_pool.starmap_async(
        check_performance_raw,
        [
            (env, deepcopy(policy), names[idx], iters, eval_dir, verbose, max_eval_steps)
            for idx, env in enumerate(random_envs)
        ],
    ).get()
    a_pool.close()
    a_pool.join()
...
def eval_teachers_on_teacher_envs(envs, policies, ex_dirs, iters, max_steps, verbose):
    ...
    su = a_pool.starmap_async(
        check_performance_raw,
        [
            (envs[idx], policies[idx], names[idx], iters, eval_dirs[idx], verbose, max_steps)
...
if __name__ == "__main__":
    ...
    if args.student_random_envs > 0:
        eval_student_on_random_envs(
            student_policy, env_sim, args.student_random_envs, eval_dir, args.iters, args.max_steps, args.verbose
        )
        exit()
    if args.teacher_eval:
        ...
        eval_teachers_on_student_env(
            env_sim, teacher_policies, teacher_ex_dirs, args.iters, args.max_steps, args.verbose
        )
